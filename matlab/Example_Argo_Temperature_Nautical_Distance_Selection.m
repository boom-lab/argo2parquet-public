...
% Filtering geographical coordinates with interactive circle selection
% Note: this needs to be done in a few stages. i) a circle is drawn; ii)
% all the data that fits in the smallest square that includes the circle
% are loaded (parquet loading cannot use ad hoc filters); iii) data outside
% the circle is discarded; iv) data further than the provided nautical
% distance are discarded too.
% i) selection
[centerCoords, radius] = selectPoint();
minLat = centerCoords(1) - radius;
maxLat = centerCoords(1) + radius;
minLon = centerCoords(2) - radius;
maxLon = centerCoords(2) + radius;
% ii) first filter (box)
...
% iii) remove data outside of circle
theta = linspace(0,2*pi,1000);
latR = centerCoords(1) + radius*sin(theta');
lonR = centerCoords(2) + radius*cos(theta');
polygonCoords = [ [latR; latR(1)], [lonR; lonR(1)] ];
rowsToRemove = outsidePolygon(dataParallel,polygonCoords);
dataParallel( rowsToRemove, : ) = [];

% iv) detect data further than prescribed nautical distance
dataParallel = filterNauticalPath( dataParallel, centerCoords, radius);

if height(dataParallel) < 1
    error('No float matches all the filters you provided: are your filters too restrictive?');
end

% v) plot showing removed floats + removing them for later plot
scatterMap(dataParallel, 'IN_RADIUS', 'Floats within prescribed radius from chosen point');
for j = height(dataParallel):-1:1
    if dataParallel.IN_RADIUS(j) == 0
        dataParallel(j,:) = [];
    end
end
dataParallel.IN_RADIUS = [];
